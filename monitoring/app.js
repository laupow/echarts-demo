document.addEventListener('DOMContentLoaded', (event) => {
	...
	function initResponsiveChart(chartDomId) {
		const chartDom = document.getElementById(chartDomId);
		const maxWidth = window.innerWidth > 600 ? 600 : window.innerWidth - 20; // Respect a small margin
		const maxHeight = maxWidth * (2 / 3); // Maintain aspect ratio of 3:2
	...
	// Initialize all charts with responsive design
	const charts = {
		loadAverageChart: loadAverageOptions,
		cpuUtilizationChart: cpuUtilizationOptions,
		memoryUtilizationChart: memoryUtilizationOptions,
		networkThroughputChart: networkThroughputOptions,
		networkLatencyChart: networkLatencyOptions,
		networkAvailabilityChart: networkAvailabilityOptions
	};
